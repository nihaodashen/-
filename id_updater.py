...
def save_session_id(session_id):
    ...
    print(f"\n📝 正在尝试将 Session ID 写入 '{CONFIG_PATH}'...")
    res = save_config_value("session_id", session_id)
    if res:
        print(f"✅ 成功更新 Session ID。")
        print(f"   - session_id: {session_id}")
    else:
        print(f"❌ 更新 Session ID 失败。请检查上述错误信息。")
        ...
        if self.path == '/update':
            try:
                ...
                if session_id:
                    ...
                    self._send_cors_headers()
                    self.end_headers()
                    self.wfile.write(b'{"error": "Missing sessionId"}')
            except Exception as e:
...
if __name__ == "__main__":
    ...
    if mode == 'battle':
        ...
        print("请注意：无论选择A或B，捕获到的 Session ID 都会更新到配置文件。")
