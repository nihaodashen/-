...
try:
    ...
    logger_temp.warning("python-dotenv 未安装，.env 文件支持将被禁用。运行: pip install python-dotenv")

# --- 内部模块导入 ---
from modules.file_uploader import upload_to_file_bed
from modules.headless_requester import get_headless_requester


# --- .env 文件操作函数 ---
ENV_FILE_PATH = ".env"
COOKIE_ENV_MAPPING = {
    'arena-auth-prod-v1.0': 'ARENA_AUTH_V1_0',
    'arena-auth-prod-v1.1': 'ARENA_AUTH_V1_1',
    'cf_clearance': 'CF_CLEARANCE'  # 保留用于发送请求，但不从浏览器同步
}

def load_cookies_from_env():
    """从 .env 文件直接读取 cookies（返回字典）"""
        ...
        with open(ENV_FILE_PATH, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if '=' in line:
                    ...
                    # 查找对应的 cookie 名称
                    for cookie_name, env_name in COOKIE_ENV_MAPPING.items():
                        if key == env_name and value:
                            cookies[cookie_name] = value
                            break
        
        if cookies:
            logger.info(f"✅ 从 .env 文件直接读取了 {len(cookies)} 个 cookies")
        
        return cookies
        
    except Exception as e:
        logger.error(f"从 .env 文件读取 cookies 时发生错误: {e}", exc_info=True)
        return {}

def save_cookies_to_env(cookies_dict: dict):
    """将 cookies 保存到 .env 文件（不使用环境变量，不添加引号）"""
    if not DOTENV_AVAILABLE:
        logger.warning("python-dotenv 未安装，无法保存到 .env 文件")
        return False
    ...
    try:
        ...
        if not os.path.exists(ENV_FILE_PATH):
            with open(ENV_FILE_PATH, 'w', encoding='utf-8') as f:
                f.write("# LMArena Bridge - Cookie Configuration\n")
                f.write("# arena-auth cookies 由油猴脚本自动更新\n")
                f.write("# cf_clearance 需要手动从 cookie.txt 复制或手动填写\n\n")
                for env_name in COOKIE_ENV_MAPPING.values():
                    f.write(f"{env_name}=\n")
        
        # 先读取现有的 cookies，以保留未更新的值
        existing_cookies = load_cookies_from_env()
        
        saved_count = 0
        for cookie_name, env_name in COOKIE_ENV_MAPPING.items():
            # 如果新的 cookies_dict 中有这个 cookie，使用新值
            # 否则使用现有值（保留不变）
            if cookie_name in cookies_dict:
                value = cookies_dict[cookie_name]
                saved_count += 1
            else:
                value = existing_cookies.get(cookie_name, '')
            
            # 使用 quote_mode='never' 不添加引号
            set_key(ENV_FILE_PATH, env_name, value or '', quote_mode='never')
        
        if saved_count > 0:
            logger.info(f"💾 已将 {saved_count} 个 cookies 保存到 .env 文件")
        
        return True
        
    except Exception as e:
        logger.error(f"保存 cookies 到 .env 文件时发生错误: {e}", exc_info=True)
        return False

# --- Cookie 获取函数 ---
def get_dynamic_cookies() -> dict:
    """
    每次都从 .env 文件直接读取最新的 cookies
    返回格式化的 cookie 字典，用于 headless_requester
    """
    # 每次都重新从文件读取
    cookies_dict = load_cookies_from_env()
    
    if cookies_dict:
        logger.debug(f"从 .env 文件获取了 {len(cookies_dict)} 个 cookies")
...
# 新增：人机验证冷却机制
VERIFICATION_COOLDOWN_START = None  # 冷却开始时间
VERIFICATION_COOLDOWN_DURATION = 30  # 冷却时长（秒）

# --- Cookie 管理（仅使用 .env 和环境变量，不使用全局变量）---
...
def save_config():
    ...
    try:
        ...
        with open('config.jsonc', 'w', encoding='utf-8') as f:
            f.write(content_str)
        logger.info("✅ 成功将 Session ID 更新到 config.jsonc。")
    except Exception as e:
...
async def convert_openai_to_lmarena_payload(openai_data: dict, session_id: str, mode_override: str = None, battle_target_override: str = None) -> dict:
    """
    将 OpenAI 请求体转换为油猴脚本所需的简化载荷，并应用酒馆模式、绕过模式以及对战模式。
    新增了模式覆盖参数，以支持模型特定的会话模式。
    注意：不再需要 message_id，因为新的 API 只需要 session_id。
    """
    # 1. 规范化角色并处理消息
    ...
    if CONFIG.get("tavern_mode_enabled"):
        logger.info("酒馆模式已启用，将所有 system 角色转换为 user 角色")
        # 将所有 system 消息转换为 user 消息
        for msg in processed_messages:
            if msg['role'] == 'system':
                msg['role'] = 'user'
                logger.debug(f"已将 system 消息转换为 user: {msg['content'][:50]}...")
    ...
    return {
        ...
        "battle_target": final_battle_target  # 添加 battle_target 参数
    }
...
async def _process_headless_stream(
    session_id: str,
    ...
    """
    核心内部生成器（无头模式）：直接通过 HTTP 请求 LMArena API，并产生结构化事件。
    事件类型: ('content', str), ('finish', str), ('error', str)
    注意：不再需要 message_id 参数。
    """
    # 传递配置和动态 cookie 获取器给 headless_requester
    headless_requester = get_headless_requester(config=CONFIG, cookie_getter=get_dynamic_cookies)
    ...
    try:
        async for event_type, data in headless_requester.send_request(
            session_id=session_id,
            message_templates=message_templates,
            target_model_id=target_model_id,
        ...
        # 流结束
        logger.info(f"[Headless] 流式数据处理完成。")
        
    except asyncio.CancelledError:
        logger.info(f"[Headless] 任务被取消。")
    except Exception as e:
        logger.error(f"[Headless] 处理流时发生错误: {e}", exc_info=True)
        yield 'error', str(e)

async def _process_lmarena_stream(request_id: str):
    """
    核心内部生成器（传统模式）：处理来自浏览器的原始数据流，并产生结构化事件。
    事件类型: ('content', str), ('finish', str), ('error', str)
    """
    ...
    try:
        while True:
            ...
            def handle_cloudflare_verification():
                global IS_REFRESHING_FOR_VERIFICATION, VERIFICATION_COOLDOWN_START
                if not IS_REFRESHING_FOR_VERIFICATION:
                    logger.warning(f"PROCESSOR [ID: {request_id[:8]}]: 首次检测到人机验证，将发送刷新指令并进入冷却期。")
                    IS_REFRESHING_FOR_VERIFICATION = True
                    VERIFICATION_COOLDOWN_START = time.time()  # 记录冷却开始时间
                    if browser_ws:
                        asyncio.create_task(browser_ws.send_text(json.dumps({"command": "refresh"}, ensure_ascii=False)))
                    return "检测到人机验证，已发送刷新指令，请等待1分钟后重试。"
                else:
...
async def stream_generator(request_id: str, model: str, headless_payload: dict = None):
    ...
    # 根据是否提供了 headless_payload 来选择处理方式
    if headless_payload:
        # 无头模式
        event_stream = _process_headless_stream(
            session_id=headless_payload['session_id'],
            message_templates=headless_payload['message_templates'],
            target_model_id=headless_payload['target_model_id'],
            is_image_request=headless_payload.get('is_image_request', False)
        )
    else:
        # 传统模式（通过 WebSocket）
        event_stream = _process_lmarena_stream(request_id)
...
async def non_stream_response(request_id: str, model: str, headless_payload: dict = None):
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if IS_REFRESHING_FOR_VERIFICATION:
        ...
        IS_REFRESHING_FOR_VERIFICATION = False
        VERIFICATION_COOLDOWN_START = None  # 重置冷却时间
...
async def chat_completions(request: Request):
    ...
    if not use_headless:
        ...
        # 检查是否在冷却期内
        if VERIFICATION_COOLDOWN_START is not None:
            elapsed = time.time() - VERIFICATION_COOLDOWN_START
            if elapsed < VERIFICATION_COOLDOWN_DURATION:
                remaining = int(VERIFICATION_COOLDOWN_DURATION - elapsed)
                raise HTTPException(
                    status_code=503,
                    detail=f"正在等待人机验证冷却期结束，请等待 {remaining} 秒后重试。"
                )
    ...
    if not session_id or "YOUR_" in session_id:
        raise HTTPException(
            status_code=400,
            detail="最终确定的 Session ID 无效。请检查 'model_endpoint_map.json' 和 'config.jsonc' 中的配置，或运行 `id_updater.py` 来更新默认值。"
        )
    ...
    try:
        ...
        if use_headless:
            # --- 无头模式 ---
            logger.info(f"API CALL [ID: {request_id[:8]}]: 使用无头模式直接发送请求。")
            
            # 直接调用流式或非流式生成器，传入 payload
            if is_stream:
                return StreamingResponse(
                    stream_generator(request_id, model_name or "default_model", headless_payload=lmarena_payload),
                    media_type="text/event-stream"
                )
            else:
                return await non_stream_response(request_id, model_name or "default_model", headless_payload=lmarena_payload)
        else:
            # --- 传统模式（通过 WebSocket） ---
            # 包装成发送给浏览器的消息
            message_to_browser = {
...
@app.post("/internal/update-cookies")
async def update_cookies(request: Request):
    """
    接收来自油猴脚本的关键 cookies 并保存到 .env 文件和环境变量
    """
    try:
        ...
        if not cookies:
            logger.warning("收到空的 cookies 数据")
            return JSONResponse(
                status_code=400,
                content={"success": False, "message": "No cookies provided"}
            )
        
        # 检查哪些 cookies 发生了变化
        updated_keys = []
        for key in ['arena-auth-prod-v1.0', 'arena-auth-prod-v1.1', 'cf_clearance']:
            if key in cookies:
                env_name = COOKIE_ENV_MAPPING.get(key)
                # 直接从 os.environ 读取，而不是 os.getenv()
                old_value = os.environ.get(env_name)
                new_value = cookies[key]
                
                logger.debug(f"Cookie 比较 {key}: 旧='{old_value[:20] if old_value else None}...' 新='{new_value[:20] if new_value else None}...'")
                
                # 只有值变化时才标记为更新
                if old_value != new_value:
                    updated_keys.append(key)
                    logger.debug(f"✓ {key} 已变化，将更新")
                else:
                    logger.debug(f"- {key} 未变化，跳过")
        
        # 总是保存到 .env 文件和环境变量（即使没有变化，也确保文件是最新的）
        save_result = save_cookies_to_env(cookies)
        
        if updated_keys:
            logger.info(f"✅ 成功更新 {len(updated_keys)} 个 cookies (来源: {source}): {', '.join(updated_keys)}")
        else:
            logger.info(f"📊 Cookies 无变化，但已确保 .env 文件同步 (来源: {source})")
        
        return JSONResponse({
            "success": True,
            "message": f"Updated {len(updated_keys)} cookies" if updated_keys else "No changes",
        ...
        logger.error(f"更新 cookies 时发生错误: {e}", exc_info=True)
        return JSONResponse(
            status_code=500,
...
@app.get("/internal/get-cookies")
async def get_cookies_status():
    """
    获取当前存储在环境变量中的 cookies（调试用）
    """
    cookies_status = {}
    for cookie_name, env_name in COOKIE_ENV_MAPPING.items():
        value = os.getenv(env_name)
        cookies_status[cookie_name] = f"{value[:20]}..." if value else None
...
async def health_check():
    ...
    return {
        "status": "ok",
        "mode": "headless" if CONFIG.get("use_headless_mode", False) else "websocket",
        "timestamp": datetime.now().isoformat()
    }
...
async def reload_cookies():
    """手动重载 cookies"""
    if not CONFIG.get("use_headless_mode", False):
        return {
            "success": False,
            "message": "此功能仅在无头模式下可用"
        }
    
    try:
        from modules.headless_requester import get_headless_requester
        requester = get_headless_requester(config=CONFIG)
        success = requester.force_reload_cookies()
        
        if success:
            return {
                "success": True,
                "message": f"成功重载了 {len(requester.cookies)} 个 cookies",
                "cookie_count": len(requester.cookies),
                "timestamp": datetime.now().isoformat()
            }
        else:
            return {
                "success": False,
                "message": "重载 cookies 失败，请检查 .env 文件",
                "timestamp": datetime.now().isoformat()
            }
    except Exception as e:
        logger.error(f"重载 cookies 时发生错误: {e}", exc_info=True)
        return {
            "success": False,
